...
| `user_id` | **Oui, pseudonymisé** | Identifiant du demandeur — utile pour détecter les redondances par le même utilisateur | Hash SHA-256 avec sel |
| `agent_id` | **Oui** | Identifiant du technicien — utile pour la ré-affectation après alerte | Conservé tel quel |
...
### 2.3 Champs optionnels (collectés seulement si disponibles)

| Champ | Condition | Justification |
|---|---|---|
| `resolution_details` | Existe dans le endpoint ? | Pourrait améliorer la détection de doublons résolus, mais peut contenir des PII |
| `custom_field_values` | Si mapping connu et non PII | À activer uniquement si utile et inoffensif |
...
| Regroupement par utilisateur | `user_id` (pseudonymisé) | `user.email`, `user.name`, téléphone |
...
2. Que `user_id` a bien été hashé avant stockage
